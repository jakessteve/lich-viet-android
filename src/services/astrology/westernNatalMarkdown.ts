...
export function formatWesternNatalAsMarkdown(result: SwissNatalChartResult): string {
  ...
  for (const object of result.objects) {
    ...
    lines.push(`| \`${object.id}\` | ${object.nameVi} | ${object.longitude.toFixed(6)} | ${object.signVi} | ${object.degree}°${object.minute.toString().padStart(2, '0')}′ | ${object.house} | ${value(object.speed)} / ${value(object.latitudeSpeed)} / ${value(object.distanceSpeed)} | ${object.retrograde === null ? '—' : object.retrograde ? 'Rx' : ''} | ${object.latitude.toFixed(6)} | ${value(object.distance)} | ${value(object.rightAscension)} | ${value(object.declination)} | ${eclipticFlags} / ${equatorialFlags} |`);
  }
  lines.push('', '## 12 nhà', '', '| Nhà | Kinh độ | Cung | Độ-phút |', '|---:|---:|---|---:|');
  for (const house of result.houses) {
    lines.push(`| Nhà ${house.number} | ${house.longitude.toFixed(6)} | ${house.signVi} | ${house.degree}°${house.minute.toString().padStart(2, '0')}′ |`);
  }
  lines.push('', '## Bốn góc chính', '', '| ID | Góc | Kinh độ | Cung | Độ-phút |', '|---|---|---:|---|---:|');
  for (const angle of Object.values(result.angles)) {
    lines.push(`| \`${angle.id}\` | ${angle.nameVi} | ${angle.longitude.toFixed(6)} | ${angle.signVi} | ${angle.degree}°${angle.minute.toString().padStart(2, '0')}′ |`);
  }
  lines.push('', '## Góc chiếu', '', '| Loại | A | B | Separation | Exact | Allowed orb | Actual orb | State | Strength | Style | Layer |', '|---|---|---|---:|---:|---:|---:|---|---:|---|---:|');
  for (const aspect of result.aspects) {
    lines.push(`| ${aspect.name} | \`${aspect.objectAId}\` | \`${aspect.objectBId}\` | ${aspect.separation.toFixed(6)} | ${aspect.exactAngle} | ${aspect.allowedOrb} | ${aspect.orbDifference.toFixed(6)} | ${aspect.state} | ${aspect.strength.toFixed(6)} | ${aspect.color}; ${aspect.opacity}; ${aspect.width}; ${aspect.dashPattern} | ${aspect.layer} |`);
  }
