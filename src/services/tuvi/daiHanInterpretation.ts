...
const TRUONG_SINH_INSIGHTS: Record<string, {
  ...
  'Mộc Dục': {
    ...
    advice: 'Giữ vững mục tiêu cốt lõi, cẩn trọng trong các quyết định tài chính bộc phát và duy trì sự tỉnh táo trước cám dỗ.',
  },
  'Quan Đới': {
    level: 'thinh',
    score: 8.5,
  ...
  'Tuyệt': {
    ...
    advice: 'Bình thản buông bỏ điều không cần thiết, tái cơ cấu mục tiêu và kiên nhẫn chuẩn bị cho một chặng đường mới.',
  },
  'Thai': {
    level: 'tich_luy',
    score: 7.5,
  ...
  'Dưỡng': {
    ...
    advice: 'Dành thời gian rèn luyện nội lực, chăm sóc các mối quan hệ gia đình và tích lũy nguồn lực sẵn sàng bứt phá.',
  },
};
...
function evaluateTamTaiMatrix(
  ...
  const hasGoodStars = phuTinh.some((s) => ['Tả Phụ', 'Hữu Bật', 'Thiên Khôi', 'Thiên Việt', 'Văn Xương', 'Văn Khúc', 'Lộc Tồn', 'Hóa Khoa'].includes(s));
  const hasBadStars = satTinh.some((s) => ['Kình Dương', 'Đà La', 'Hỏa Tinh', 'Linh Tinh', 'Địa Không', 'Địa Kiếp', 'Hóa Kỵ'].includes(s));
  ...
  if (hasGoodStars && !hasBadStars) {
    ...
    nhanHoaDesc = 'Vừa có quý nhân nâng đỡ vừa có sự cạnh tranh gay gắt; cần khéo léo trong đối nhân xử thế để biến thử thách thành động lực.';
  } else if (hasBadStars) {
    nhanHoaScore = 5.0;
  ...
  const totalScore = Math.round(
    (thienThoiScore * 0.3 + diaLoiScore * 0.25 + nhanHoaScore * 0.25 + tsInfo.score * 0.2) * 10,
  ) / 10;
...
function calculateDaiHanTuHoa(palaceCan: Can, chart: TuViChart): DaiHanInterpretationResult['daiHanTuHoa'] {
  ...
  if (natalLoc && natalLoc.starName === mapping.Loc) {
    interactions.push(`Song Lộc Trùng Phùng: Đại Hạn Hóa Lộc gặp Năm Sinh Hóa Lộc tại sao ${mapping.Loc} — Thời vận đại phát tài lộc.`);
  }
  if (natalKy && natalKy.starName === mapping.Ky) {
    interactions.push(`Song Kỵ Trùng Phùng: Đại Hạn Hóa Kỵ gặp Năm Sinh Hóa Kỵ tại sao ${mapping.Ky} — Cần hết sức thận trọng thị phi, giấy tờ và rủi ro lớn.`);
  }
...
function evaluateTamPhuongTuChinh(
  ...
  const tamHopPalaces = [th1?.name ? `${th1.name} (${th1.chi})` : '', th2?.name ? `${th2.name} (${th2.chi})` : ''].filter(Boolean);
  ...
  const projectingTuHoa = related.flatMap((p) =>
    p.tuHoa.map((th) => `Hóa ${th.type} (${th.starName}) tại ${p.name}`),
  );
  const projectingAuspicious = related.flatMap((p) =>
    p.phuTinh.filter((s) => ['Tả Phụ', 'Hữu Bật', 'Văn Xương', 'Văn Khúc', 'Thiên Khôi', 'Thiên Việt', 'Lộc Tồn', 'Thiên Mã', 'Đào Hoa', 'Hồng Loan'].includes(s.name)).map((s) => `${s.name} (${p.name})`),
  );
  const projectingMalefics = related.flatMap((p) =>
    p.satTinh.map((s) => `${s.name} (${p.name})`),
  );
...
function detectProminentPatterns(
  ...
  const allProjectingStars = [...tptc.projectingMajorStars, ...tptc.projectingAuspicious, ...tptc.projectingMalefics].map((s) => s.split(' ')[0]);
  ...
  const hasHoaKy = palace.tuHoa.some((th) => th.type === 'Kỵ') || tptc.projectingTuHoa.some((th) => th.includes('Hóa Kỵ'));
  ...
  if (hasLocTon && hasHoaLoc) {
    patterns.push({
      ...
      type: 'cat',
      description: 'Lộc Tồn và Hóa Lộc cùng hội tụ: Đại hạn bội thu tài chính, kinh doanh buôn bán đại phát, mở rộng tài sản nhanh chóng.',
  ...
  if ((hasLocTon || hasHoaLoc) && hasThienMa) {
    patterns.push({
      ...
      type: 'cat',
      description: 'Lộc Tồn/Hóa Lộc gặp Thiên Mã: Đại hạn làm ăn phương xa, giao thương quốc tế, càng dịch chuyển công tác càng gặt hái tài lộc dồi dào.',
  ...
  if (hasHoaKhoa && hasHoaQuyen && hasHoaLoc) {
    patterns.push({
      ...
      type: 'cat',
      description: 'Khoa – Quyền – Lộc hội đủ trong đại vận: Thời kỳ danh vọng vang dội, quyền uy vững chắc, thi cử thăng tiến rực rỡ.',
  ...
  if (allDomainStars.includes('Thiên Khôi') || allDomainStars.includes('Thiên Việt')) {
    patterns.push({
      ...
      type: 'cat',
      description: 'Gặp Thiên Khôi/Thiên Việt trong đại hạn: Dễ gặp quý nhân bề trên nâng đỡ, thi cử đỗ đạt, đạt thành tựu vẻ vang.',
      note: 'Gặp Thiên Khôi/Thiên Việt trong đại hạn: Dễ gặp quý nhân bề trên nâng đỡ, thi cử đỗ đạt, đạt thành tựu vẻ vang.',
    });
  }

  // Đào Hồng Hỷ
  if (allDomainStars.includes('Đào Hoa') || allDomainStars.includes('Hồng Loan') || allDomainStars.includes('Thiên Hỉ')) {
    patterns.push({
      name: 'Đào Hồng Hỷ Tín',
      type: 'cat',
      description: 'Đào Hoa, Hồng Loan, Thiên Hỷ hội chiếu: Đại hạn có nhiều tin vui gia đạo, tình duyên nảy nở, nhân duyên xã hội rộng mở.',
  ...
  if (presentMalefics.length >= 2) {
    patterns.push({
      ...
      type: 'hung',
      description: 'Có từ 2 sát tinh trở lên hội tụ: Cần cẩn trọng trong các cam kết tài chính, đề phòng va chạm tranh chấp và giữ gìn thể trạng sức khỏe.',
  ...
  if (hasHoaKy) {
    patterns.push({
      ...
      type: 'hung',
      description: 'Hóa Kỵ xuất hiện trong đại vận: Cảnh báo khẩu thiệt thị phi, hiểu lầm trong hợp đồng văn bản; cần giữ sự điềm tĩnh và làm việc minh bạch.',
  ...
  if (allDomainStars.includes('Đại Hao') || allDomainStars.includes('Tiểu Hao')) {
    patterns.push({
      ...
      type: 'trung',
      description: 'Đại vận có Song Hao: Dòng tiền luân chuyển mạnh, tiêu pha lớn cho đầu tư, học hành hoặc thay đổi nơi ở, xuất ngoại.',
  ...
  if (allDomainStars.includes('Cô Thần') || allDomainStars.includes('Quả Tú')) {
    patterns.push({
      name: 'Cô Quả Độc Lập',
      type: 'trung',
      description: 'Có Cô Thần/Quả Tú: Vận trình thiên về tự lực gánh vác, độc lập nghiên cứu chuyên môn sâu sắc, ít người đồng điệu tâm sự.',
...
function getLifeStageTheme(startAge: number): { stageName: string; coreFocus: string } {
  if (startAge < 20) {
    return {
      ...
      coreFocus: 'Tập trung rèn luyện nhân cách, học vấn thi cử, xây dựng nền tảng tri thức và giữ gìn thể trạng tuổi trẻ.',
    };
  }
  if (startAge < 40) {
    return {
      stageName: 'Thanh Xuân & Khởi Nghiệp',
      coreFocus: 'Giai đoạn lập thân, định hình sự nghiệp, tạo dựng các mối quan hệ then chốt, hôn nhân gia đạo và tích lũy ban đầu.',
    };
  }
  if (startAge < 60) {
    return {
      stageName: 'Trung Niên & Đỉnh Cao',
      coreFocus: 'Khẳng định vị thế xã hội, mở rộng quy mô sự nghiệp, quản trị dòng tiền đầu tư và chăm lo cho thế hệ kế cận.',
    };
  }
  return {
    stageName: 'Vãn Niên & An Dưỡng',
    coreFocus: 'An hưởng thành quả cuộc đời, chăm sóc sức khỏe thể chất lẫn tinh thần, làm việc thiện tích phước và truyền thừa kinh nghiệm.',
  };
}
...
function evaluatePhasingBreakdown(
  ...
  if (palace.hasTriet) {
    firstHalf += 'Do có Triệt Không án ngữ, 5 năm đầu có thể gặp nhiều thử thách chông gai, đòi hỏi sự bền bỉ tôi luyện bản lĩnh. ';
    secondHalf += 'Vượt qua thử thách ban đầu, 5 năm sau vận trình chuyển hướng hanh thông, công việc khai hoa kết trái rực rỡ. ';
 ...
 */
export function interpretDaiHan(
  palace: TuViPalace,
  chart: TuViChart,
  isCurrent = false,
): DaiHanInterpretationResult {
  ...
  const tamTai = evaluateTamTaiMatrix(
    menhHanh,
    palaceChiHanh,
    ttName,
    tsName,
  ...
  if (isVoChinhDieu) {
    ...
    clusterSummary = 'Cung Vô Chính Diệu: Vận thế biến ảo, linh hoạt theo ngoại cảnh, đắc lực khi biết liên kết nguồn lực bên ngoài.';
  } else {
    ...
    if (hasSatPha && !hasTuPhu) {
      ...
      clusterSummary = 'Cục diện Sát Phá Tham: Vận trình khai phá, cải cách và đổi mới dũng mãnh. Dám nghĩ dám làm sẽ mở ra bước ngoặt lớn.';
    } else if (hasTuPhu && !hasSatPha) {
      clusterType = 'tu_phu_vu_tuong';
      clusterSummary = 'Cục diện Tử Phủ Vũ Tướng: Vận trình quyền uy vững chắc, chủ về tích lũy tài sản, mở rộng cơ nghiệp và củng cố uy danh.';
    } else if (hasCoNguyet && !hasSatPha) {
      clusterType = 'co_nguyet_dong_luong';
      clusterSummary = 'Cục diện Cơ Nguyệt Đồng Lương: Vận trình mưu lược, trí tuệ, bình ổn, thích hợp cho giáo dục, dịch vụ và quản trị.';
    } else if (hasCuNhat) {
      clusterType = 'cu_nhat';
      clusterSummary = 'Cục diện Cự Nhật: Vận trình đối ngoại, truyền thông, tài ăn nói và danh tiếng; cần cẩn trọng lời nói để tránh thị phi.';
    } else {
      clusterType = 'hon_hop';
  ...
  if (palace.hasTuan && palace.hasTriet) {
    tuanTrietNote = 'Cung vị ngộ cả Tuần lẫn Triệt: Vận thế lúc đầu nhiều biến động thử thách, nhưng về sau xoay chuyển ngoạn mục.';
  } else if (palace.hasTriet) {
    tuanTrietNote = 'Cung vị có Triệt Không: Nửa đầu đại hạn (5 năm đầu) cần kiên trì vượt khó, nửa sau vận trình hanh thông rộng mở.';
  } else if (palace.hasTuan) {
    tuanTrietNote = 'Cung vị có Tuần Không: Vận thế chuyển biến êm ả, bảo toàn bình an và giảm bớt hung tính của các sát tinh.';
  }
  ...
  if (prominentPatterns.some((p) => p.type === 'cat')) {
    const catNotes = prominentPatterns.filter((p) => p.type === 'cat').map((p) => p.name).join(', ');
  ...
  if (prominentPatterns.some((p) => p.name.includes('Đào Hồng'))) {
    relationshipAndHealth += 'Đại vận đón nhận hỷ tín tình cảm, nhân duyên gia đạo nảy nở và mở rộng mạng lưới bạn bè tốt đẹp. ';
  }
  if (tsName === 'Bệnh' || tsName === 'Tử' || palace.name === 'Tật Ách' || satTinhNames.length >= 2) {
    relationshipAndHealth += 'Cần chú ý lắng nghe cơ thể, duy trì lối sống lành mạnh, cân bằng công việc và tránh để áp lực công việc ảnh hưởng đến sức khỏe thể chất. ';
  } else {
    relationshipAndHealth += 'Tinh thần duy trì được sự lạc quan, gia đạo êm ấm, tạo hậu phương vững chắc để bạn yên tâm phát triển sự nghiệp. ';
  }
  ...
  return {
    ...
    elementalAnalysis: {
      ...
      relationType: tamTai.diaLoi.level === 'Tương Sinh' ? 'sinh_nhap' : tamTai.diaLoi.level === 'Tỷ Hòa' ? 'ty_hoa' : tamTai.diaLoi.level === 'Khắc Xuất' ? 'khac_xuat' : tamTai.diaLoi.level === 'Sinh Xuất' ? 'sinh_xuat' : 'khac_nhap',
