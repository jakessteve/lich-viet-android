...
export const VedicDiamondChart: React.FC<{ result: WesternChartResult, type: 'D1' | 'D9' }> = ({ result, type }) => {
  ...
  const H = [
    { i: 1, points: `${S/2},0 ${S/4},${S/4} ${S/2},${S/2} ${3*S/4},${S/4}`, tx: S/2, ty: S/4 + 10 },
    { i: 2, points: `0,0 ${S/4},${S/4} ${S/2},0`, tx: S/4, ty: S/8 + 10 },
    { i: 3, points: `0,0 0,${S/2} ${S/4},${S/4}`, tx: S/8 + 5, ty: S/4 + 10 },
    { i: 4, points: `0,${S/2} ${S/4},${3*S/4} ${S/2},${S/2} ${S/4},${S/4}`, tx: S/4 + 10, ty: S/2 },
    { i: 5, points: `0,${S} ${S/4},${3*S/4} 0,${S/2}`, tx: S/8 + 5, ty: 3*S/4 - 10 },
    { i: 6, points: `0,${S} ${S/2},${S} ${S/4},${3*S/4}`, tx: S/4, ty: 7*S/8 - 10 },
    { i: 7, points: `${S/2},${S} ${3*S/4},${3*S/4} ${S/2},${S/2} ${S/4},${3*S/4}`, tx: S/2, ty: 3*S/4 - 10 },
    { i: 8, points: `${S},${S} ${3*S/4},${3*S/4} ${S/2},${S}`, tx: 3*S/4, ty: 7*S/8 - 10 },
    { i: 9, points: `${S},${S} ${S},${S/2} ${3*S/4},${3*S/4}`, tx: 7*S/8 - 5, ty: 3*S/4 - 10 },
    { i: 10, points: `${S},${S/2} ${3*S/4},${S/4} ${S/2},${S/2} ${3*S/4},${3*S/4}`, tx: 3*S/4 - 10, ty: S/2 },
  ...
  return (
    <div style={{display:'flex',justifyContent:'center'}} data-vedic-diamond-chart>
      <svg viewBox={`0 0 ${S} ${S}`} style={{width:'100%',maxWidth:S}}>
        ...
        <polygon points={`${S/2},0 0,${S/2} ${S/2},${S} ${S},${S/2}`} fill="none" stroke={co.ln} strokeWidth="1.2" />
        ...
        {H.map(h => {
          ...
          return (
            <g key={'h'+h.i}>
              <text x={h.tx} y={h.ty - 15} textAnchor="middle" fontSize="10" fontWeight="700" fill={co.tx} opacity={0.4}>
                {signForHouse}
              </text>
              {planets.map((pl, pi) => {
                ...
                return (
                  <text key={pl.b} x={px} y={py} fontSize="11" fontWeight="800" fill={SC[pl.b]||co.tx} textAnchor="middle">
                    {SI[pl.b]}
                  </text>
                );
              })}
