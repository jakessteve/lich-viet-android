  ...
  return (
    ...
    >
      <div className="card-header border-b border-[#374e68]/40 pb-2 mb-4">
        <h3 className="section-title text-sm flex items-center gap-2">Góc hợp & Tọa độ</h3>
      </div>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <tbody>
              {uniqueBodies.map((body) => (
                <tr
                  key={`legend-${body.id}`}
                  className="border-b border-[#374e68]/50 last:border-0 hover:bg-white/5 transition-colors"
                >
                  <td
                    className="py-1 px-2 w-8 text-center text-lg font-bold"
                    style={{ color: BODY_COLORS[body.id] || '#fff' }}
                  >
                    {BODY_ICONS[body.id] || body.id.substring(0, 2)}
                  </td>
                  <td className="py-1 px-2 text-[#8eb0d1]">{BODY_LABELS[body.id] || body.id}</td>
                  <td className="py-1 px-2 font-medium" style={{ color: '#8eb0d1' }}>
                    {body.sign} {formatDegMin(body.longitude)}
                    {body.retrograde ? ' R' : ''}
                  </td>
                </tr>
              ))}
