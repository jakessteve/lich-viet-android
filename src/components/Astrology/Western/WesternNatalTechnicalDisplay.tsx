...
const Section: React.FC<{
  ...
  <details className="group border-t border-border-light/50 first:border-t-0 dark:border-border-dark/50">
    ...
    <div className="border-t border-border-light/40 bg-surface-container-lowest/40 dark:border-border-dark/40">{children}</div>
  </details>
);
...
export const WesternNatalTechnicalDisplay: React.FC<{ result: SwissNatalChartResult }> = ({ result }) => {
  ...
  return (
    <section className="surface-card overflow-hidden rounded-2xl border border-border-light/60 text-xs dark:border-border-dark/60" data-western-natal-technical>
      <button
        type="button"
        className="flex min-h-14 w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-indigo-500/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-indigo-500"
        aria-expanded={open}
      ...
      {open && (
        <div id="western-natal-technical-content" className="border-t border-border-light/50 dark:border-border-dark/50">
              ...
              {result.objects.map((object) => (
                <li key={object.id} data-technical-object={object.id} className="p-3 sm:p-4">
                  ...
                  <dl className="grid grid-cols-2 gap-x-3 gap-y-2 sm:grid-cols-4">
                    ...
                    <Value label="Speed">{n(object.speed)}{object.retrograde ? ' Rx' : object.retrograde === null ? ' · unknown' : ''}</Value>
                    <Value label="Latitude">{object.latitude.toFixed(6)}°</Value>
                    <Value label="Distance">{n(object.distance)} AU</Value>
                    <Value label="Lat / dist speed">{n(object.latitudeSpeed)} / {n(object.distanceSpeed)}</Value>
                    <Value label="RA / Dec">{n(object.rightAscension)}° / {n(object.declination)}°</Value>
                    <Value label="Flags ecl / eq">{result.metadata.returnedFlags[object.id] ?? '—'} / {result.metadata.returnedEquatorialFlags[object.id] ?? '—'}</Value>
                  </dl>
                </li>
              ))}
            </ul>
          ...
          <Section title="Góc chiếu" count={result.aspects.length}>
            <ul className="divide-y divide-border-light/40 dark:divide-border-dark/40">
              {visibleAspects.map((aspect, index) => (
                <li key={`${aspect.objectAId}-${aspect.objectBId}-${aspect.id}-${index}`} data-technical-aspect={aspect.id} className="p-3 sm:px-4">
                  ...
                  <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2 sm:grid-cols-4">
                    <Value label="Separation / exact">{aspect.separation.toFixed(6)}° / {aspect.exactAngle}°</Value>
                    <Value label="Orb / allowed">{aspect.orbDifference.toFixed(6)}° / {aspect.allowedOrb}°</Value>
                    <Value label="State / strength">{aspect.state} / {aspect.strength.toFixed(6)}</Value>
                    <Value label="Style / layer">{aspect.color} · {aspect.opacity} · {aspect.width} · {aspect.dashPattern} · L{aspect.layer}</Value>
                  </dl>
                </li>
              ))}
            </ul>
            {result.aspects.length > ASPECT_PREVIEW_LIMIT && (
              <div className="border-t border-border-light/40 p-3 text-center dark:border-border-dark/40">
                <button
                  ...
                  className="min-h-11 rounded-xl px-4 font-semibold text-indigo-700 transition-colors hover:bg-indigo-500/10 dark:text-indigo-300"
                >
          ...
          <Section title="Metadata kỹ thuật">
            <dl className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4" data-technical-metadata>
              ...
              <Value label="Engine">{result.metadata.engine} {result.metadata.version}</Value>
              <Value label="Ephemeris">{result.metadata.ephemeris}</Value>
              <Value label="Requested flags">{result.metadata.requestedFlags} / eq {result.metadata.requestedEquatorialFlags}</Value>
              <Value label="Policy">{result.metadata.objectPolicyVersion} · {result.metadata.aspectPolicyVersion}</Value>
              <Value label="Time policy">{result.metadata.timePolicy}</Value>
              <Value label="Part of Fortune" wide>{result.metadata.partOfFortuneAltitudePolicy} · solar altitude {result.metadata.partOfFortuneSolarAltitudeDeg.toFixed(6)}°</Value>
            </dl>
          </Section>
        </div>
      )}
