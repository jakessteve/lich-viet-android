...
const BODY_COLORS: Record<string, string> = {
  ...
  neptune: '#3498db', pluto: '#bdc3c7', chiron: '#3498db', lilith: '#bdc3c7',
  northnode: '#9b59b6', southnode: '#9b59b6', partOfFortune: '#e67e22', vertex: '#bdc3c7',
  ascendant: '#bdc3c7', midheaven: '#bdc3c7'
};
  ...
  return (
    <div className="glass-card overflow-hidden mt-6 p-4 rounded-xl shadow-lg border border-[#374e68]/40" style={{ backgroundColor: '#152336', color: '#fff' }}>
      <div className="card-header border-b border-[#374e68]/40 pb-2 mb-4 text-center">
        <h3 className="section-title text-sm flex items-center justify-center gap-2">
          Xích vĩ (Declination)
        </h3>
      </div>
      ...
      <div className="overflow-x-auto">
        <svg width="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ minWidth: '600px' }}>
          ...
          {/* North/South Labels */}
          <text x={PAD_X - 15} y={PAD_Y - 40} textAnchor="end" dominantBaseline="central" fill="#8eb0d1" fontSize="14">Bắc</text>
          <text x={PAD_X - 15} y={PAD_Y + 40} textAnchor="end" dominantBaseline="central" fill="#8eb0d1" fontSize="14">Nam</text>
          ...
          {[0, 5, 10, 15, 20, 25, 30].map(tick => {
            ...
            return (
              <g key={`tick-${tick}`}>
                ...
                <text x={tx} y={30} textAnchor="middle" fill="#8eb0d1" fontSize="12">{tick}°</text>
                <text x={tx} y={H - 25} textAnchor="middle" fill="#8eb0d1" fontSize="12">{tick}°</text>
              </g>
            );
          })}
          ...
          {bodies.map((body, i) => {
            ...
            return (
              <g key={`dec-${body.id}-${i}`}>
                <line x1={x} y1={PAD_Y} x2={x} y2={yEnd} stroke={BODY_COLORS[body.id] || '#fff'} strokeWidth="1.5" opacity={0.8} />
                <text 
                  x={x} 
                  y={yEnd + (dir * 15)} 
                  textAnchor="middle" 
