...
const SIGNS_VI = ['Bạch Dương', 'Kim Ngưu', 'Song Tử', 'Cự Giải', 'Sư Tử', 'Xử Nữ', 'Thiên Bình', 'Bọ Cạp', 'Nhân Mã', 'Ma Kết', 'Bảo Bình', 'Song Ngư'];
...
export function createWesternNatalFixture(): SwissNatalChartResult {
  ...
  const makeAngle = (id: string, name: 'Ascendant' | 'Descendant' | 'Midheaven' | 'Imum Coeli', nameVi: string, symbol: string, longitude: number) => ({
    id,
    name,
    nameVi,
    symbol,
