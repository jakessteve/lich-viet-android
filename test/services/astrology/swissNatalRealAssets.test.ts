...
import { calculateSwissNatalChart, initializeBundledSwissNatalEphemeris } from '@/services/astrology/swissNatalChart';
...
describe('bundled Swiss browser integration', () => {
  ...
  beforeAll(async () => {
    server = createServer(async (request, response) => {
      try {
        ...
        const file =
          pathname === '/swisseph.wasm'
            ? resolve('node_modules/@swisseph/browser/dist/swisseph.wasm')
            : resolve('public', pathname.slice(1));
        const data = await readFile(file);
        response.writeHead(200, {
          'Content-Type': pathname.endsWith('.wasm') ? 'application/wasm' : 'application/octet-stream',
        });
        response.end(data);
      } catch {
  ...
  it('loads the real WASM and local ephemeris bundle for a fractional fixed offset', async () => {
    ...
    expect(
      result.objects
        .slice(0, 13)
        .every((object) => Number.isFinite(object.rightAscension) && Number.isFinite(object.declination)),
    ).toBe(true);
